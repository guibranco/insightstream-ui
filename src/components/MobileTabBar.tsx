...
export const MobileTabBar: React.FC = () => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-[#0A0B0D]/95 backdrop-blur-md border-t border-slate-200/80 dark:border-white/10 px-2 py-1.5 flex items-center justify-around shadow-lg">
      {tabs.map((tab) => {
        const isActive =
        ...
        return (
          <a
            ...
            className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-colors min-w-[60px] text-center ${
              isActive
                ? 'text-[#79378B] dark:text-[#93CD3F] font-bold'
